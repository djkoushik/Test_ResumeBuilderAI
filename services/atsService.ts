...
const scoreFromCanonical = (
    candidateCanonical: Set<string>,
    ...
    if (jdCriticalCanonical.size > 0) {
        ...
        const bonusPart = jdBonusCanonical.size > 0 ? (matchCountBonus / jdBonusCanonical.size) : 0;
    ...
    if (jdText) {
        ...
        const candidateString = candidateText.toLowerCase();
...
export const calculateATSScore = (
    ...
    return scoreFromCanonical(candidateCanonical, candidateYears, hasEducation, candidateText, jobDescription);
};

export const calculateATSScoreFromText = (
    candidateText: string,
    jobDescription: JobDescriptionInput
): ATSScoreResult => {
    ...
    // extract skills from text using standardSkills list
    const extractedSkills: string[] = [];
    standardSkills.forEach(skill => {
        if (normalizedCandidateText.includes(skill)) extractedSkills.push(skill);
    });
    // check aliases
    Object.keys(canonicalMap).forEach(key => {
        if (normalizedCandidateText.includes(key)) extractedSkills.push(canonicalMap[key]);
    });

    const candidateSet = normalizeSkills(extractedSkills);
    const candidateCanonical = mapToCanonical(candidateSet);

    // Estimate experience from text (very rough)
    // Look for "X years" patterns? Difficult on raw text without structure.
    // For now, assume a default if we can't find it, or try to find "Total Experience"
    let candidateYears = 0;
    const expMatch = normalizedCandidateText.match(/(\d+)[\+]*\s*years/i);
    if (expMatch) {
        candidateYears = parseInt(expMatch[1]);
    }
    // Try to find sections to be smarter? 
    // Implementing a fallback parser here is complex. 
    // Let's assume some basic checks.

    const hasEducation = normalizedCandidateText.includes('education') || normalizedCandidateText.includes('university') || normalizedCandidateText.includes('college') || normalizedCandidateText.includes('degree');
